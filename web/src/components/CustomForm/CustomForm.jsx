...
class CustomForm extends Component {
  ...
  handleSubmit(event) {
    ...
    docRef.set({ ...this.state, coords: coords || 'undefined' });
                ...
                {struct ? (
                  struct.map((s, key) => {
                    if (s.type === 'select')
                      return (
                        <GridItem xs={12} sm={12} md={8} key={key}>
                          <CustomSelect
                            ...
                            labelProps={
                              s.labelProps || { shrink: Boolean(state[s.id]) }
                            }
                            options={s.options}
                      ...
                      return (
                        <GridItem xs={12} sm={12} md={8} key={key}>
                          <InputLabel htmlFor={s.id}>{s.labelText}</InputLabel>
                          <Checkbox
                            checked={state[s.id]}
                            onChange={event =>
                              this.setState({ [s.id]: event.target.checked })
                            }
                            name={s.id}
                            id={s.id}
                          />
                        ...
                        <GridItem xs={12} sm={12} md={8} key={key}>
                          <CustomInput
                            ...
                            inputProps={{
                              ...
                              onChange: handleChange,
                              type: s.type,
                              endAdornment: Boolean(s.endAdornment) && (
                                <InputAdornment position="end">
                                  {s.endAdornment}
                                </InputAdornment>
                              ),
